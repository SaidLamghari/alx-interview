...
// `./0-starwars_characters.js 3`, 'movieId' vaudra '3'
...
const api_lnk = `https://swapi.dev/api/films/${mveId}/`;

// Envoi d'une requête GET à l'API pour récupérer les détails du film
request(api_lnk, async function (error, response, body) {
    // Vérification de l'existence d'une erreur dans la requête
    if (error) return console.log(error);

    // Analyse du corps de la réponse pour
    // obtenir les données sous forme d'objet JSON
    ...
    for (const link of characters) {
        ...
        await new Promise((resolve) => {
            ...
            request(link, (error, response, body) => {
                ...
                // pour obtenir les détails du personnage
